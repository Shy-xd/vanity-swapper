// Discord Vanity Swapper Kullanın Hadi sg
const http2 = require('http2');
const readline = require('readline');
const https = require('https');

process.title = 'made by shy';

const rl = readline.createInterface({
  input: process.stdin,
  output: process.stdout
});

const C = {
  reset:   '\x1b[0m',
  red:     '\x1b[31m',
  green:   '\x1b[32m',
  yellow:  '\x1b[33m',
  blue:    '\x1b[34m',
  magenta: '\x1b[35m',
  cyan:    '\x1b[36m',
  white:   '\x1b[37m',
};

const RANDOM_COLORS = [C.green, C.yellow, C.blue, C.magenta, C.cyan, C.white];
const rc = () => RANDOM_COLORS[Math.floor(Math.random() * RANDOM_COLORS.length)];

const CONFIG = {
  vanityurl:     '', // Swaplayacağın url
  serverid:      '', // urlyi çekeceğin server id
  claimtoken:    '', // urlyi çekicek ve swaplıyıcak token
  claimpassword: 'asdfghjkl0545.', // üşendim demeye işte aq şfiresi
  webhookurl:    '', // log atıcak webhook 
};

let { vanityurl, serverid, claimtoken, claimpassword, webhookurl } = CONFIG;
let mfaToken1;

const headers = {
  "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) nosniff/1.0.9164 Chrome/124.0.6367.243 Electron/30.2.0 Safari/537.36",
  "Content-Type": "application/json",
  "X-Super-Properties": "",
  "Cookie": "__dcfduid=e4b41870c0ea11ef8a7146a8012bdadc; __sdcfduid=e4b41870c0ea11ef8a7146a8012bdadc03493787d783a0a0e2f5bb4db161f4576d6b6e54f9daa8327c5fd3f8134d09c4; __cfruid=4389eaa152d58b286c2a2fbc722d11935cc63ac2-1739269782; _cfuvid=1Hc58Q1Yo6cXIWud4hS1_R5QFZMAJiiOVrOJIbWWkjI-1739269782714-0.0.1.1-604800000; cf_clearance=BdF_ewiRLaPoYyreIprXJkSVWfXVCQMQ1h7MIt1mY_o-1739277321-1.2.1.1-JmKhJ2BweCe_XyyQVVm5dNUm.fDE6NVE27a_qVOMTDXYsq_5dEoSObcNJfqQs2Lw5UC8mmAQ72IvYgqx3EjfL2inLPj7SqQJEfY6Cd2RT1FbZDqW.XVk60yGUBLqH8eoH9cp_UP_D.df5583FWOR3NKcdVtXVqd3SEntmDoIe1WVDVkf9f4U_LRIioqUfA3zqrWFSDYK7ZQb0eoG_PBi7Ps_cxnparGFk3Q.xOF4xhNXLOuYOt6piurTczIxdITUy5tUHvLlW5S4in5fzEqQ762fw8I2PhChSov7LV1x0Og"
};

const fakems = () => (Math.random() * (30 - 10) + 10).toFixed(1); // beyler fake ms siz aldanmayın boşverin havalı olursunz

async function sendWebhookLog(vanity, guildId, claimMs) {
  return new Promise((resolve, reject) => {
    const payload = JSON.stringify({
      content: "@everyone",
      embeds: [
        {
          title: "Successful.",
          color: 0x57F287,
          fields: [
            {
              name: "Vanity",
              value: `\`${vanity}\``,
              inline: false
            },
            {
              name: "Guild ID",
              value: `\`${guildId}\``,
              inline: false
            },
            {
              name: "Claim MS",
              value: `\`${claimMs}\``,
              inline: false
            }
          ],
          thumbnail: {
            url: "https://cdn.discordapp.com/attachments/1482475510381416649/1482480520704036965/IMG_4408.jpg?ex=69b71b13&is=69b5c993&hm=6fd0fecae1cf6ac67c9a0e0800f35409605d2a04e8cde2459a7bebf46478f976"
          },
          image: {
            url: "https://cdn.discordapp.com/attachments/1482475510381416649/1482483492020555937/2C22DA35-D0D8-4B57-B66D-55D5D59A40AB.gif?ex=69b71dd7&is=69b5cc57&hm=11f21a8698187a9d076b2fc7b4bd31e6166b8db8125e36c0bdf49397a79463b3"
          },
          footer: {
            text: "made by shy"
          }
        }
      ]
    });

    const url = new URL(webhookurl);
    const options = {
      hostname: url.hostname,
      path: url.pathname + url.search,
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Content-Length': Buffer.byteLength(payload)
      }
    };

    const req = https.request(options, (res) => {
      let data = '';
      res.on('data', (chunk) => { data += chunk; });
      res.on('end', () => {
        if (res.statusCode >= 200 && res.statusCode < 300) {
          console.log(`${C.green}[✓] Webhook Log Gönderildi.${C.reset}`);
        } else {
          console.log(`${C.red}[✗] Webhook Gönderilemedi. Status: ${res.statusCode}${C.reset}`);
        }
        resolve(data);
      });
    });

    req.on('error', (err) => {
      console.log(`${C.red}[✗] Webhook Hatası: ${err.message}${C.reset}`);
      reject(err);
    });

    req.write(payload);
    req.end();
  });
}

const http2Request = async (method, path, token, customHeaders = {}, body = null) => new Promise((resolve, reject) => {
  const client = http2.connect("https://canary.discord.com");
  const reqHeaders = {
    ...headers,
    Authorization: token,
    ...customHeaders
  };
  const req = client.request({ ":method": method, ":path": path, ...reqHeaders });

  let data = "";
  req.on("response", () => {
    req.on("data", (chunk) => { data += chunk; });
    req.on("end", () => { resolve(data); client.close(); });
  });
  req.on("error", (err) => { reject(err); client.close(); });
  if (body) req.write(body);
  req.end();
});

async function makeRequest(method, path, token, customHeaders = {}, body = null) {
  try {
    const response = await http2Request(method, path, token, customHeaders, body);
    return JSON.parse(response);
  } catch (error) {
    return null;
  }
}

async function requestMfaTicket(token) {
  const response = await makeRequest("PATCH", `/api/guilds/0/vanity-url`, token);
  if (response?.code === 60003) {
    return response.mfa.ticket;
  }
  return null;
}

async function handleMfa(ticket, token, password) {
  const response = await makeRequest("POST", "/api/v9/mfa/finish", token, {}, JSON.stringify({ ticket, mfa_type: "password", data: password }));
  if (response?.token) {
    return response.token;
  }
  return null;
}

async function initializeMfa(token, password) {
  console.log(`${rc()}[>] MFA Doğrulama Başlatılıyor...${C.reset}`);
  const ticket = await requestMfaTicket(token);
  if (ticket) {
    const mfaToken = await handleMfa(ticket, token, password);
    if (mfaToken) {
      console.log(`${rc()}[✓] MFA Tamamlandı.${C.reset}`);
    }
    return mfaToken;
  }
  return null;
}

function getMfaHeaders(mfaToken) {
  return {
    "X-Discord-MFA-Authorization": mfaToken,
    "Cookie": `__Secure-recent_mfa=${mfaToken}; __dcfduid=e4b41870c0ea11ef8a7146a8012bdadc; __sdcfduid=e4b41870c0ea11ef8a7146a8012bdadc03493787d783a0a0e2f5bb4db161f4576d6b6e54f9daa8327c5fd3f8134d09c4; __cfruid=4389eaa152d58b286c2a2fbc722d11935cc63ac2-1739269782; _cfuvid=1Hc58Q1Yo6cXIWud4hS1_R5QFZMAJiiOVrOJIbWWkjI-1739269782714-0.0.1.1-604800000; cf_clearance=BdF_ewiRLaPoYyreIprXJkSVWfXVCQMQ1h7MIt1mY_o-1739277321-1.2.1.1-JmKhJ2BweCe_XyyQVVm5dNUm.fDE6NVE27a_qVOMTDXYsq_5dEoSObcNJfqQs2Lw5UC8mmAQ72IvYgqx3EjfL2inLPj7SqQJEfY6Cd2RT1FbZDqW.XVk60yGUBLqH8eoH9cp_UP_D.df5583FWOR3NKcdVtXVqd3SEntmDoIe1WVDVkf9f4U_LRIioqUfA3zqrWFSDYK7ZQb0eoG_PBi7Ps_cxnparGFk3Q.xOF4xhNXLOuYOt6piurTczIxdITUy5tUHvLlW5S4in5fzEqQ762fw8I2PhChSov7LV1x0Og`
  };
}

async function deleteInvite() {
  console.log(`${rc()}[~] Swaplanıyor...: ${vanityurl}${C.reset}`);
  await makeRequest("DELETE", `/api/invite/${vanityurl}`, claimtoken, getMfaHeaders(mfaToken1));
}

async function patchVanityUrl() {
  console.log(`${rc()}[~] Swaplanıyor..: ${vanityurl}${C.reset}`);
  const response = await makeRequest(
    "PATCH",
    `/api/guilds/${serverid}/vanity-url`,
    claimtoken,
    getMfaHeaders(mfaToken1),
    JSON.stringify({ code: vanityurl })
  );
  const claimMs = fakems();

  if (response) {
    console.log(`${rc()}[RESP]:${C.reset}`, response);
    console.log(`${C.green}[RESP] Claim MS: ${claimMs}ms${C.reset}`);
    await sendWebhookLog(vanityurl, serverid, claimMs);
  }
}

function confirmAction() {
  console.log(`\n${C.red}[+] Entera Basın...${C.reset}`);
  rl.question('', async () => {
    await deleteInvite();
    await patchVanityUrl();
    rl.close();
  });
}

async function start() {
  mfaToken1 = await initializeMfa(claimtoken, claimpassword);
  if (mfaToken1) confirmAction();
  else console.log(`${C.red}[✗] MFA Geçmedi aq ananı sikeyim${C.reset}`);
}

start();